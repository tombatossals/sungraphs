...
export default function InverterDailyChart({ data, color }: Props) {
  const intervals = Object.values(data.intervals).filter(
    interval =>
      typeof interval.p1 === "number" && typeof interval.p2 === "number"
  );

  if (intervals.length === 0) return null;

  const labels = intervals.map(i => i.iso_time.slice(11, 16));
  const values = intervals.map(i => (i.p1 ?? 0) + (i.p2 ?? 0));
